...
% ── Notation macros (consistent with PythonicDISORT; see online docs) ────
...
\begin{abstract}
This report documents \texttt{pydisort\_riccati\_jax}, a differentiable
forward solver for radiative transfer in atmospheres where the
single-scattering albedo $\omega(\tau)$ and phase function vary
continuously with optical depth.  Instead of per-layer eigendecomposition,
the solver integrates the invariant-imbedding Riccati ODE for reflection and
transmission operators using diffrax's Kvaerno5 method~\cite{kidger2022neural}
(5-stage ESDIRK, L-stable, order~5) with PIDController adaptive step-size
control, running on JAX~\cite{jax2018github} for automatic differentiability.
All terms in the ODE have positive sign---no growing exponentials appear
anywhere in the computation, guaranteeing unconditional numerical stability
for arbitrary optical depth.  We derive the complete system of ODEs
(reflection~$\mat{R}$, transmission~$\mat{T}$, beam source~$\vect{s}$) for
both forward and backward sweeps, present the boundary-condition solve, and
discuss the Kvaerno5 integration via JAX and diffrax.  The forward solver is
composed with a differentiable Mie lookup table (\texttt{miejax\_lite}) and a
Rodgers optimal-estimation retrieval loop, forming a complete end-to-end
differentiable pipeline for retrieving effective-radius profiles $r_e(\tau)$
from multi-band, multi-angle ToA radiance observations; the retrieval is
validated on VOCALS-REx marine stratocumulus profiles.  This report extends
Sections~3--4 of the Pythonic-DISORT Comprehensive
Documentation~\cite{HP2024doc} and assumes familiarity with that document.
\end{abstract}
...
\textbf{Delta-$M$ scaling and the Nakajima--Tanaka TMS correction are
implemented} as opt-in features (\cref{sec:deltaM}); the
\emph{intensity multiple-scattering} (IMS) correction is omitted.  This is
appropriate for a ToA-upwelling observable: in DISORT itself the IMS correction
applies to \emph{downward intensities only by construction}~\cite{STWLE2000}, so
there is no standard upward IMS to apply, and the retrieval-grade codes
LIDORT/VLIDORT likewise omit it (exact single scatter + streams, plus a separate
second-order-of-scattering model).  The remaining features of
PythonicDISORT~\cite{HP2024doc} below are either structural differences or not
yet implemented.
...
%----------------------------------------------------------------------
\subsection{Delta-$M$ scaling and the TMS correction}\label{sec:deltaM}
%----------------------------------------------------------------------

For a forward-peaked cloud phase function ($g_1\!\approx\!0.85$), truncating the
Legendre expansion at $N_{\mathrm{Leg}}$ moments makes the higher azimuthal
Fourier modes ring and the reconstructed ToA radiance go \emph{negative}, even
though the angle-integrated flux and the $m=0$ mode stay positive.  The standard
remedies---delta-$M$ scaling~\cite{Wiscombe1977} and the Nakajima--Tanaka TMS
single-scattering correction---are provided as opt-in features
(\texttt{delta\_M\_scaling}, \texttt{NT\_cor}; both default off, so the un-scaled
numerics and tests are unchanged).

\paragraph{Physical-$\tau$ delta-$M$.}
Let $f(\tau)=g_{N_{\mathrm{Leg}}}(\tau)$ be the truncation fraction (the first
dropped moment; derived internally, never a separate input).  Rather than
reparametrise to the scaled depth, we keep integrating in \emph{physical} $\tau$
and absorb the Jacobian $\mathrm{d}\tau^\ast/\mathrm{d}\tau=s(\tau)$, with
$s(\tau)=1-\omega(\tau)f(\tau)$, into the coefficients.  Writing the
\emph{effective moments} $c_\ell=g_\ell-f$, the only changes to the
operators~\eqref{eq:alpha-beta}--\eqref{eq:Qm} are
\begin{align}
  \mat{\alpha}_{\mathrm{d}M} &= \mat{M}^{-1}\!\big(\omega\,\mat{D}^{+}[c]\,\mat{W}
      - s\,\mat{I}\big), &
  \mat{\beta}_{\mathrm{d}M} &= \mat{M}^{-1}\,\omega\,\mat{D}^{-}[c]\,\mat{W},
  \nonumber\\
  \vec{q}^{\pm}_{\mathrm{d}M} &= \mat{M}^{-1}\,c_{\mathrm{fac}}\,\omega\,
      e^{-\tau^\ast(\tau)/\mu_0}\,\vec{X}^{\pm}[c], &
  \tau^\ast(\tau) &= \int_0^\tau \!\big(1-\omega f\big)\,\mathrm{d}\tau' ,
  \label{eq:deltaM}
\end{align}
i.e.\ $c_\ell$ replaces $g_\ell$, the identity term carries the depth-scale
$s(\tau)$, and the beam attenuation uses the \emph{scaled} cumulative depth
$\tau^\ast$.  $f=0$ recovers the un-scaled solver exactly.  $\tau^\ast$ is the
only cumulative (non-pointwise) quantity and is azimuth-mode-independent, so it
is built once before the Fourier loop.  Only $e^{-\tau^\ast/\,\cdot}$ with
$\tau^\ast\!\ge\!0$ appears, preserving the no-positive-exponents invariant
(\cref{sec:bc}).

\paragraph{TMS correction.}
Delta-$M$ smooths the multiple-scattering field; TMS restores the single
scattering with the \emph{exact}, untruncated phase function.  Mapping
PythonicDISORT's per-layer correction to the continuum gives the upwelling ToA
correction as a single one-dimensional $\tau$-quadrature,
\begin{equation}
  \Delta u^{+}_i(\phi) = \frac{I_0}{4\pi\mu_i}\!\int_0^{\tau_{\mathrm{bot}}}\!
    \omega(\tau)\,\big[p_{\mathrm{full}} - (1-f)\,p_{\mathrm{trunc}}\big]
      \!\big(\cos\Theta_i(\phi)\big)\,
    e^{-\tau^\ast(\tau)\,(1/\mu_0 + 1/\mu_i)}\,\mathrm{d}\tau ,
  \label{eq:tms}
\end{equation}
whose angular bracket collapses to the \emph{missing forward-peak detail}
$\sum_{\ell<N_{\mathrm{Leg}}}(2\ell+1)f\,P_\ell
  + \sum_{\ell\ge N_{\mathrm{Leg}}}(2\ell+1)g_\ell\,P_\ell$
at the exact scattering-angle cosine $\cos\Theta_i(\phi)$ between the incident
beam $(-\mu_0,\phi_0)$ and the outgoing $(+\mu_i,\phi)$.  The integrand is smooth
(the angular sharpness lives in $\cos\Theta$, evaluated exactly), so a fixed
$\sim\!128$-point Gauss--Legendre rule suffices; the exponent $1/\mu_0+1/\mu_i$
is a \emph{sum} (no $\mu_i=\mu_0$ singularity for upwelling, invariant
preserved).  TMS corrects the intensity only---the flux and $u_0$ remain the
delta-$M$ $m=0$ quantities---and, being analytic in $\mu$, is added directly at
the requested polar angle when interpolating rather than interpolated through the
quadrature nodes.  The solver matches PythonicDISORT's own \texttt{NT\_cor}
solution to $\sim\!3\times10^{-7}$ for a homogeneous layer (float64).

\paragraph{Residual at extreme peaks.}
TMS is a single-scattering-only correction, so for phase functions far
under-resolved by the stream count it does not guarantee strictly non-negative
radiance: the delta-$M$-scaled, $N$-stream \emph{multiple}-scattering field can
retain a small negative lobe (measured: H--G $g=0.9$ at NQuad$=16$, raw min
$-0.13$ reduced to $-0.025$).  This residual is present in PythonicDISORT's
\texttt{NT\_cor} as well---it is intrinsic to truncated-basis discrete
ordinates, not this implementation.  For realistic cloud asymmetry
($g\!\approx\!0.85$) the radiance is non-negative; for sharper peaks the residual
is most simply removed by \emph{more streams} (cheap here, since the solve is
jit-able and batchable across columns).  The Nakajima--Tanaka IMS correction is
\emph{not} the remedy---it is downward-only by construction in
DISORT~\cite{STWLE2000} and is likewise omitted by LIDORT/VLIDORT.  The
$\delta$-$M$+ truncation~\cite{LinStamnes2018} is an alternative that improves the
truncated representation, but it is confined to the DISORT/AccuRT ecosystem and
its same-cost claim does not carry over to a differentiable forward model (it
replaces the trivial $f=g_{N_{\mathrm{Leg}}}$ slice with a per-layer nonlinear
moment-matching solve in the autodiffed path); it is not pursued here.
...
top.  When the thin layer $\dd\sigma$ is added at the top, light entering at
the bottom first transmits up through the existing slab ($\mat{T}$), then
undergoes one back-reflection cycle between the slab (reflecting via
$\mat{R}$) and the new thin layer (reflecting via $r = \mat{\beta}\dd\sigma$),
and finally exits upward through the thin layer
($t = \mat{I} + \mat{\alpha}\dd\sigma$):
\begin{align}
  \mat{T}(\sigma + \dd\sigma)
  &= t\,(\mat{I} + \mat{R}\,r)\,\mat{T} + O(\dd\sigma^2) \nonumber \\
  &= (\mat{I} + \mat{\alpha}\dd\sigma)\,
     (\mat{I} + \mat{R}\,\mat{\beta}\dd\sigma)\,\mat{T}
     + O(\dd\sigma^2) \nonumber \\
  &= \mat{T} + (\mat{\alpha} + \mat{R}\,\mat{\beta})\,\mat{T}\dd\sigma
     + O(\dd\sigma^2).
\end{align}
Taking the limit:
\begin{equation}\label{eq:companion-T}
  \boxed{
    ...
    = (\mat{\alpha} + \mat{R}\,\mat{\beta})\,\mat{T},
    \qquad \mat{T}(0) = \mat{I}.
  }
\end{equation}
...
%----------------------------------------------------------------------
\subsection{Solver Lineage and the Step-Count Objective}\label{sec:lineage}
%----------------------------------------------------------------------

The choice of an implicit, adaptive integrator was reached by elimination,
and the dead ends are worth recording because they are tempting to retry.

\paragraph{Magnus + Redheffer star product (stable, but step count pinned
to the ballistic eigenvalue).}
An earlier solver propagated the discrete-ordinate field with a
first-order Magnus integrator, accumulating $N\!\times\!N$
reflection/transmission/source operators through the Redheffer star
product rather than a $2N\!\times\!2N$ propagator.  This is
\emph{unconditionally stable} --- it works entirely with $O(1)$-bounded
slab operators and never forms the coexisting growing and decaying modes
of the full propagator (the same motivation as the no-positive-exponents
invariant, \cref{sec:stability}).  Its weakness is the \emph{step count}:
each Magnus step must resolve the largest eigenvalue of the coefficient
matrix.  The eigenvalues are the discrete-ordinate fingerprints of the
two classical radiative-transfer regimes.  The largest, the
\emph{ballistic eigenvalue} $\lambda_{\max}\approx 1/\mu_{\min}\approx 45$
(NQuad${}=16$, $\mu_{\min}$ the smallest quadrature abscissa), is the
single-scattering regime --- the rapid extinction of the grazing stream
before the field isotropises.  The smallest, the \emph{diffusion
eigenvalue} $k_{\min}\approx\sqrt{3(1-\omega)(1-\omega g)}$ (the two-stream
diffusion exponent; $\approx 0.07$ at $\omega=0.99$, falling to
$\approx 0.02$ at $\omega=0.999$), is the diffusion domain --- the slow
asymptotic decay $e^{-k_{\min}\tau}$ of the multiply-scattered field deep
in a thick cloud.  The step count is set by the ballistic eigenvalue,
$K \gtrsim \lambda_{\max}\,\tbot$, while the \emph{observable} evolves on
the far slower diffusion scale; the explicit method therefore vastly
over-resolves the physics.  For $\tbot=50$, $\omega=0.99$, $g=0.85$ this
is $\approx 2272$ steps.

\paragraph{Three-domain decomposition (discarded: no step saving).}
The natural fix was to split $[0,\tbot]$ into a non-diffusive top (beam
active), a diffusion interior (beam negligible), and a non-diffusive
near-surface layer, solve each independently, and recombine with the
(associative) star product --- the aim being to step coarsely through the
interior.  It was \textbf{discarded}: the interior still required fine
Magnus steps, so the split produced essentially no saving
($462+914+914 = 2290$ steps vs $2272$ for a single Magnus solve, the
same wall time) for added bookkeeping.

\paragraph{Resolution: one implicit stiff solver everywhere.}
Integrating the Riccati system \eqref{eq:rhs-R}--\eqref{eq:rhs-s} with an
\emph{implicit} (L-stable) method steps at the \emph{diffusion} rate
across the whole column, with no domain split: $\approx 35$ adaptive
steps for a $\tbot=30$ cloud (\cref{sec:step-count}), versus thousands
for Magnus.  It is both simpler (a single solver, no domain boundaries to
choose) and far cheaper in step count.  Retained from the discarded work
are the Redheffer star product / interaction principle --- the
layer-composition rule that yields the Riccati increment
(\cref{sec:forward}) and the final up/down combination in the
boundary-condition solve (\cref{sec:bc}) --- and the $O(1)$-bounded
$N\!\times\!N$ operator formulation.

\paragraph{Step count as a design objective.}
Beyond simplicity, the minimal step count is itself a primary design
goal --- not for run time, but because the converged ODE grid is the only
a-priori-available, problem-derived, profile-adaptive candidate for the
grid on which $r_e(\tau)$ is retrieved, and a \emph{minimal} such grid is
what makes it usable for an observable with few degrees of freedom.  This
motivation, and how the retrieval grid relates to the ODE grid, is
developed in \cref{sec:retrieval-grid}.
...
the ballistic/diffusion eigenvalue gap of $\mat{A}$ (\cref{sec:lineage})
spans a wide range, and the Riccati nonlinearity amplifies it, so an
...
The solver runs in \textbf{float32 by default} ($\texttt{tol} \approx
10^{-3}$), which keeps the adaptive step count low and is more than
adequate for retrieval, where accuracy is set by $\sim$10--20\%
measurement noise rather than float resolution.  Float64
(\texttt{jax.config.update("jax\_enable\_x64", True)}, opt-in via the
environment flag \texttt{PYDISORT\_RICCATI\_JAX\_X64=1}) is reserved for
tight-tolerance work---resolving transmission operators $\mat{T}$ that
decay to $O(10^{-13})$ in thick atmospheres, and finite-difference
gradient checks.  In float64, tolerances of $10^{-8}$--$10^{-10}$ are
reachable; in float32, driving \texttt{tol} below $\sim$$10^{-4}$ on
thick atmospheres makes the PIDController shrink the step indefinitely
(a controller failure, not an instability).
...
\subsubsection{Step count behaviour}\label{sec:step-count}
...
\paragraph{Delta-$M$ scaling and Nakajima--Tanaka TMS correction.}
For forward-peaked phase functions (e.g.\ Mie scattering with
$g\!\gtrsim\!0.8$), the truncated Legendre expansion introduces a
negative-radiance artifact in the single-scattering field.  The solver
implements Wiscombe's delta-$M$ scaling~\cite{Wiscombe1977} (replacing
the forward peak with a delta-function transport term) and the
Nakajima--Tanaka TMS correction (replacing the truncated
single-scattering source with the exact phase function), both opt-in via
\texttt{delta\_M\_scaling=True, NT\_cor=True}.  IMS (intensity correction
for the \emph{downward} field) is omitted by design, since only the
upwelling ToA field is the retrieval observable.

\paragraph{Composable seam for jit-able retrieval.}
The forward model exposes a three-function composable seam for use inside
the retrieval loop:
\begin{enumerate}[nosep]
  \item \texttt{riccati\_setup(NQuad, I0, phi0, mu0, \ldots)}
    --- host-side, run once per solar geometry; returns a frozen
    \texttt{setup} struct.
  \item \texttt{riccati\_solve(setup, omega\_func, leg\_func, tau\_bot, \ldots)}
    --- JAX-traceable; runs all Fourier modes under \texttt{lax.scan}.
  \item \texttt{eval\_radiance(setup, result, mu\_obs, phi\_obs)}
    --- assembles the observable.
\end{enumerate}
The \texttt{lax.scan} over Fourier modes compiles the mode body
\emph{once} with $O(1)$ memory in the mode count, resolving an earlier
OOM at \texttt{NQuad}$\geq 24$ when the modes were unrolled.  $\mu_0$
(cosine of solar zenith angle) is static: it enters the Legendre
products and beam source and is fixed at setup time; changing solar
geometry requires rebuilding the setup struct (one cheap recompile).

%======================================================================
\section{Retrieval: Discrete Adjoints via JAX}\label{sec:retrieval}
%======================================================================

The forward solver serves as the differentiable core of an iterative
retrieval of the effective-radius profile $r_e(\tau)$ from
top-of-atmosphere radiance observations.  This section describes the
automatic-differentiation infrastructure, the retrieval-grid analysis,
the profile parameterisation, and the implemented optimal-estimation
loop.
...
\subsubsection{Concrete example: VOCALS marine stratocumulus retrieval}

The implemented retrieval uses $p = 4$--$7$ parameters (effective-radius
values at QRCP-selected $\tau$-nodes, plus $\tau_{\mathrm{bot}}$ and
$r_{\mathrm{base}}$; see \cref{sec:parameterisation}) and $m \approx 6$--$9$
observations (3 bands $\times$ 2--3 viewing angles).  With $K \approx 50$
Kvaerno5 steps and forward-mode cost $c_f \approx 2$--$3$, the full
$m \times p$ Jacobian via \texttt{jax.jacfwd} costs
$\approx p\,c_f\,C_{\mathrm{fwd}} \approx 20\,C_{\mathrm{fwd}}$.  The
reverse-mode alternative (\texttt{RecursiveCheckpointAdjoint}) would cost
$\approx m\,c_r\,C_{\mathrm{fwd}} \approx 30$--$45\,C_{\mathrm{fwd}}$ due
to segment recomputation.  \textbf{Forward mode is preferred} for
$p < m$ or $p \approx m$, which is the case for the node-based
parameterisation; the crossover at roughly
$p \approx (c_r / c_f)\, m \approx 15$--$20$ is well above the
implemented parameter count.

In all cases, $C_{\mathrm{fwd}} \propto K$, so minimising the
integration step count (\cref{sec:stability}) directly reduces the cost
of every Jacobian computation.  Each Gauss--Newton iteration requires one
Jacobian plus one forward evaluation; the retrieval typically converges in
3--8 iterations.
...
\begin{description}[style=nextline, leftmargin=2em]
  ...
  \item[\texttt{ForwardMode}~(used for the retrieval)]
    Forward-mode AD through the solver (\texttt{jax.jvp}).  Preferred
    when $p \leq m$ (few parameters), which is the case for the
    implemented node-based retrieval ($p = 4$--$7$, $m = 6$--$9$).
    Requires \texttt{riccati\_setup(\ldots, adjoint=diffrax.ForwardMode())}
    since the default \texttt{RecursiveCheckpointAdjoint} registers a
    \texttt{custom\_vjp} that cannot be forward-differentiated.
...
\begin{enumerate}
  ...
  \item \textbf{Sensitivity accumulation over many steps.}  The discrete
    ...
    to parameters at a particular $\tau$-layer (e.g.\ near cloud top,
    where the ToA observable is most sensitive; \cref{sec:retrieval-grid}),
    this sensitivity is amplified
...
This validation has been performed: $\nabla_{\!\vect{x}} J$ computed via
\texttt{jax.grad} through \texttt{pydisort\_riccati\_jax} for a synthetic
$r_e(\tau)$ profile agrees with centred finite-difference gradients to a
relative error of ${\sim}2\times 10^{-10}$ (float64, step size
$h = 10^{-5}$), confirming both the correctness of the AD pipeline and
the absence of numerical pathologies in the backward pass.

%----------------------------------------------------------------------
\subsection{The Retrieval Grid and Its Relation to the ODE Grid}\label{sec:retrieval-grid}
%----------------------------------------------------------------------

A retrieval must decide \emph{at which optical depths} $r_e$ is treated
as a free unknown --- the \emph{retrieval grid}.  The ideal grid places
degrees of freedom where the observable is sensitive, but that is
\emph{a posteriori} information, unavailable before the retrieval is run.
The only a-priori-available, problem-derived, profile-adaptive candidate
is the converged \emph{ODE grid} (\cref{sec:step-count}).  This
subsection makes precise how the two grids relate.  The short statement
is: \textbf{the ODE grid is a trustworthy candidate \emph{pool}, and the
retrieval grid is a sensitivity-selected \emph{subset} of it} --- neither
the whole grid, nor a separately prescribed grid.

This is the reason \textbf{minimising the ODE step count
(\cref{sec:lineage}) is a primary design objective}: with so few
retrievable degrees of freedom (below) --- the concrete expression of
the ill-conditioned Jacobian $\mat{K}$ of \cref{sec:retrieval-problem}
--- the candidate pool must be as tight and well-placed as possible.  The implicit, L-stable solver
delivers exactly that --- a minimal grid that, by virtue of L-stability,
clusters on \emph{genuine} variation rather than stiffness artifacts, so
it is at once minimal and a trustworthy superset of the informative
depths.

\subsubsection{Information is ToA-weighted with finite penetration}

Reverse-mode AD through the full solve (\texttt{jax.jacrev} of the ToA
field with respect to piecewise-linear perturbations of $\omega(\tau)$,
$g(\tau)$ on a uniform $K=61$-node grid, $m=0$) gives the column
$\ell_2$-sensitivity $\lVert \partial u^+_0(\tau{=}0)/\partial
p(\tau_j)\rVert_2$ across the $N=8$ streams.  It decays
\emph{monotonically and steeply} with depth (adiabatic cloud,
$\omega\!\sim\!0.85$--$0.96$, drizzle feature at $\tau{=}15$,
$\tbot{=}30$):

\begin{center}
\begin{tabular}{l c c}
\toprule
Region & $\lVert\partial u^+_0/\partial\omega\rVert_2$
       & $\lVert\partial u^+_0/\partial g\rVert_2$\\
\midrule
ToA ($\tau<5$)               & $5.1\times10^{-2}$ & $5.3\times10^{-2}$\\
Mid ($5\!\leq\!\tau\!<\!13$)  & $8.4\times10^{-4}$ & $6.2\times10^{-4}$\\
Feature ($13\!\leq\!\tau\!<\!17$) & $2.1\times10^{-5}$ & $1.6\times10^{-5}$\\
...
The ToA-to-BoA ratio is ${\sim}10^{-6}$: a perturbation
$\delta\omega=0.01$ at BoA shifts $u^+_0(\mathrm{ToA})$ by
${\sim}2\times10^{-8}$ relative to $|u^+_0|\approx0.05$ --- far below any
realistic measurement noise.  (At depth the signal $J\!\cdot\!\varepsilon
\sim 10^{-8}$ also falls below the solver noise floor, so finite
differences cannot even validate the AD Jacobian there --- itself a
confirmation of the insensitivity.)  The decay is stream-dependent:
the steepest quadrature stream ($\mu_1\!\approx\!0.02$) is attenuated
ToA$\to$BoA by ${\sim}4\times10^{-9}$ and the shallowest
($\mu_8\!\approx\!0.98$) by ${\sim}4\times10^{-7}$, since steeper streams
traverse more optical path ($\propto\tau/\mu$) and scatter more on the
way up.  This is the \emph{adjoint} face of the contraction property of
\cref{sec:stability}.  The qualifier ``finite penetration'' matters: for
\emph{thin} clouds the whole column is in view and the deep region is not
prior-dominated; the steep decay above is a thick-cloud statement.

\subsubsection{Superset, not equality: why the whole grid cannot be used}

The two grids are governed by \emph{different criteria}, and that is why
they differ.  A $\tau$-point is \emph{placed} on the ODE grid because the
Riccati \emph{state} $(\mat{R},\mat{T},\vect{s})$ varies fast there (the
error controller watches the state).  A $\tau$-point is \emph{selected}
onto the retrieval grid because the observable is \emph{sensitive to the
optics} there (QRCP watches $\partial u_\mathrm{ToA}/\partial$optics).

\paragraph{Selection (pruning).}  This obeys one rule: sensitivity decays
monotonically with depth (the adjoint face of contraction; the table
above).  So a deep point is unlikely to survive QRCP \emph{regardless of
why it was placed}.

\paragraph{Placement (supply).}  Because the integrator is L-stable, the
grid clusters by \emph{genuine} state variation, not stiffness artifacts,
so it resolves every genuine feature and is a trustworthy \emph{superset}
of the informative depths --- it cannot miss a feature the solver stepped
on.  But ``genuine state variation'' is \emph{not} the same as ``optics
variation'' or ``information'': the state varies for two reasons, and
only one is retrievable.
\begin{enumerate}[nosep]
  \item \textbf{Optics features} --- $\omega(\tau)$, $g(\tau)$ changing
    with depth.  Retrievable (if shallow enough to be visible).
  \item \textbf{The BoA imbedding boundary layer} --- in the forward
    sweep $\mat{R}$ relaxes from its initial condition toward
    $\mat{R}_\infty$ over the bottom stretch (contraction \emph{forgetting
    the IC}; the steepest stream settles ${\sim}50\times$ faster than the
    shallowest), so the controller clusters steps there \emph{even for
    perfectly constant optics}.  This is genuine motion of the state but
    carries no optics content and no retrieval information.
\end{enumerate}

So the ODE grid is \emph{not an information map}: part of what it resolves
(the IC boundary layer) is informationally empty.  This is the same
contraction property seen from both sides --- its \emph{primal} face
(forgetting the IC) over-resolves BoA on the supply side, while its
\emph{adjoint} face (attenuating upward perturbations) drives the
depth-decay of sensitivity on the selection side.  The boundary layer is
the worst case: the grid is \emph{densest exactly where it is least
informative}, which is precisely why the retrieval grid must be a
sensitivity-selected subset and can never reuse the ODE grid wholesale.

\paragraph{The variation\,$\supseteq$\,information\,$\supseteq$\,retrievable
hierarchy.}  The three sets at play are strictly nested:
\begin{equation}\label{eq:hierarchy}
  \underbrace{\bigl\{\text{retrievable from ToA radiance}\bigr\}}_{\text{retrieval grid}}
  \;\subseteq\;
  \bigl\{\text{information}\bigr\}
  \;\subseteq\;
  \underbrace{\bigl\{\text{variation}\bigr\}}_{\text{ODE grid}} .
\end{equation}
\emph{Variation} is where the Riccati \emph{state} changes --- what the
ODE grid resolves.  \emph{Information} is where the \emph{optics}
genuinely change, i.e.\ where a real profile feature exists to be known.
\emph{Retrievable} is the subset of those features the ToA observable is
actually sensitive to.  The two inclusions are exactly the two faces of
contraction:
\begin{itemize}[nosep]
  \item \textbf{variation $\setminus$ information} = the BoA imbedding
    boundary layer --- state motion with \emph{no} optics cause (the
    \emph{primal}-contraction gap, $\mat{R}$ forgetting its IC);
  \item \textbf{information $\setminus$ retrievable} = deep optics
    features that genuinely exist but are attenuated to invisibility at
    ToA (the \emph{adjoint}-contraction gap).
\end{itemize}
The ODE grid resolves the \emph{outer} set; the retrieval must recover
the \emph{inner} one.  Equating them --- using the ODE grid as the
retrieval grid --- conflates all three levels and is the error the
sensitivity selection removes.

The mismatch is quantified directly on a conservative $\tbot=30$ cloud,
with and without a near-ToA $g$-spike:
...
\quad informative DoF there      & 1    & 1\\[3pt]
Solver steps near ToA ($\tau<5$) & 5    & 4\\
\quad informative DoF there      & 4--6 & 4--6\\
\bottomrule
\end{tabular}
\end{center}

Near BoA the solver spends ${\sim}14$ steps where the data constrain
${\sim}1$ quantity (a $14\times$ over-resolution of an invisible
transient); near ToA it spends $4$--$5$ steps where several independent
quantities exist (marginal).  \textbf{So the retrieval grid must be a
sensitivity-weighted \emph{subset} of the ODE pool, not the pool
itself.}  Greedy column-pivoted QR (QRCP) on the ToA Jacobian, forced to
anchor at BoA, confirms it: all $N=8$ selected depths fall in
$\tau\in[0,3.5]$, none from $\tau>3.5$, and every column at $\tau\geq5$
has cosine similarity $\geq0.9999$ with the BoA column.

\subsubsection{What is \emph{not} claimed: rank and profile-independence}

The supplementary analyses additionally measured (i) an angular
\emph{rank} of the ToA Jacobian of ${\approx}4$ (1\% singular-value
threshold) to $6$ (0.1\%), with the mid/BoA blocks collapsing to rank~1;
and (ii) approximate \emph{profile-independence} of this structure under
a Gaussian-spike position sweep.  We record these as observations but
\textbf{deliberately do not assert them as established}, for three
reasons.  First, the rank is threshold-dependent and bounded above only
by the stream count ($N=8$ at NQuad${}=16$); there is no symmetry or
analytic argument forcing ``4'', so it should be read as ``a handful,''
not a derived ceiling.  Second, the profile-independence sweep was run on
a near-conservative atmosphere, a biasing condition; in the absorbing
case the frozen mode ratios shift with the absorption profile
($|c_2/c_1|\approx0.30$ vs $0.11$) and the angular collapse moves deeper
($\tau\approx13$ vs $\approx5$).  Third, the claim that the structure is
profile-independent sits uneasily with the companion finding that
near-ToA features \emph{are} visible (a $\tau{=}1$ spike is captured and
drives the grid); detectability clearly depends on feature depth.  These
two questions --- a rigorous DoF bound, and the conditions for
profile-independence --- are left open.

\subsubsection{Consequences for the retrieval grid}

The robust consequences, independent of the exact DoF count, are:
\begin{itemize}[nosep]
  \item Build the retrieval grid as a \textbf{sensitivity/QRCP-selected
    subset} of the (minimal, L-stable) ODE pool; the minimality of the
    pool is what conditions this selection well.
  \item Parameterise $r_e(\tau)$ with a \emph{few shape parameters}
    (\cref{sec:parameterisation}), not a fine level-by-level grid, and
    place what resolution exists near cloud top \emph{by sensitivity} ---
    not by assumed profile curvature (adiabatic curvature is steepest at
    the \emph{base}, where the observable is weakest).
  \item The deep $r_e$ is prior-dominated, not measured (for thick
    clouds).
\end{itemize}

An alternative route \emph{decouples} the retrieval grid from the ODE
grid by parameterising $r_e(\tau)$ in a fixed smooth basis (EOF/PCA, an
adiabatic shape, or a neural decoder), evaluated at whatever $\tau$ the
solver requests.  Such bases are largely unexplored beyond the adiabatic
assumption, and they carry their own a-priori commitments: a basis
encodes its assumption in the chosen shape or training distribution and
is fixed rather than profile-adaptive.  Here we instead pursue the
ODE-pool route, whose complementary weakness is that it inherits the
\emph{first-guess} profile's bias in where the pool is placed --- a bias
correctable by lagged grid re-selection (re-run the forward model,
re-place the pool).  Which route is preferable is an open question; we do
not settle it here.
...
The retrieval parameterises $r_e(\tau)$ as a piecewise function of
\emph{normalised optical depth} $s = \tau / \tau_{\mathrm{bot}}$
($s \in [0,1]$, cloud-top to cloud-base), with the internal
representation in $r_e^5$ (fifth power of effective radius) for
physical consistency with the adiabatic growth law.

\subsubsection{$r_e^5$-linear parameterisation}

In a strictly adiabatic liquid cloud, $r_e^5(s) = r_{e,\mathrm{base}}^5
\cdot s$ (linear in normalised depth).  This motivates representing the
profile as piecewise linear in $(s, r_e^5)$ space: the state vector
\begin{equation}\label{eq:state}
  \vect{x} = \bigl[\,r_{e,1},\; r_{e,2},\; \ldots,\; r_{e,K},\;
  r_{\mathrm{base}},\; \tau_{\mathrm{bot}}\,\bigr]
\end{equation}
contains $r_e$ values at $K$ QRCP-selected $s$-nodes (the
sensitivity-selected subset of the ODE pool described in
\cref{sec:retrieval-grid}), plus the cloud-base effective radius
$r_{\mathrm{base}}$ and the total cloud optical depth
$\tau_{\mathrm{bot}}$.  The profile is reconstructed by
piecewise-linear interpolation of $r_e^5$ between nodes, with
extrapolation below the deepest node using the adiabatic slope; the
$r_e^5 \to r_e$ transformation is applied pointwise.  The normalised
depth ensures that the node positions adapt automatically when
$\tau_{\mathrm{bot}}$ changes during the iteration.

\subsubsection{Grounded prior}

The \emph{a~priori} state $\vect{x}_a$ and covariance $\mat{S}_a$ are
designed for marine stratocumulus following Rodgers~\cite{rodgers2000}:
\begin{itemize}[nosep]
  \item The prior profile follows the adiabatic $r_e^5$-linear law with
    a climatological $r_{\mathrm{base}}$.
  \item $\mat{S}_a$ is tight where the observable is blind (deep nodes,
    $r_{\mathrm{base}}$) and loose where it is sensitive (near-ToA nodes),
    reflecting the depth-dependent information content of
    \cref{sec:retrieval-grid}.
  \item $\tau_{\mathrm{bot}}$ has a separate, relatively tight prior
    informed by the broadband constraint.
\end{itemize}
The asymmetric prior structure ensures the retrieval is anchored at
cloud base (where the data provide essentially no constraint for thick
clouds) while allowing the near-ToA profile to depart freely from the
adiabatic assumption.

\subsubsection{Joint retrieval and the Jacobian decomposition}

The state vector~\eqref{eq:state} is jointly retrieved via
Gauss--Newton optimal estimation~\cite{rodgers2000} with the full
$m \times p$ Jacobian computed by \texttt{jax.jacfwd} (forward mode;
\cref{sec:ad}).  The differentiable chain is
$r_e(\tau) \xrightarrow{\texttt{miejax\_lite}} (\omega, g_\ell)
\xrightarrow{\texttt{riccati\_solve}} u^+(\mathrm{ToA})$, where
\texttt{miejax\_lite} provides a JAX-traceable Mie lookup table
$r_e \to (\omega, g_\ell)$.

A Jacobian decomposition experiment separating the $\omega$ and $g_\ell$
channels (by freezing one while tracing the other) reveals that the
$r_e$ signal in the ToA radiance is \emph{predominantly carried by the
phase function} (asymmetry parameter $g$), not by $\omega$, across all
tested cloud thicknesses.  This is the opposite of the common
``absorption retrieval'' intuition and has implications for the choice of
Legendre truncation (\texttt{NLeg\_all}).

\subsubsection{Posterior diagnostics}

The averaging kernel $\mat{A} = (\mat{K}^\top \mat{S}_\epsilon^{-1}
\mat{K} + \mat{S}_a^{-1})^{-1} \mat{K}^\top \mat{S}_\epsilon^{-1}
\mat{K}$ and its trace (degrees of freedom for signal, DOFS) characterise
the information content of the retrieval.  Shannon information content
(SIC) $= \tfrac{1}{2}\ln|\mat{A}| + \mathrm{const}$ provides a
complementary scalar measure; it is more sensitive to the \emph{number}
of independently constrained parameters, while DOFS is more sensitive
to the \emph{strength} of each constraint.  Both are computed from the
autodiff Jacobian at each iteration.

%----------------------------------------------------------------------
\subsection{Implementation Status}\label{sec:status}
%----------------------------------------------------------------------

All components of the differentiable retrieval pipeline are implemented
and validated:

\begin{enumerate}[nosep]
  \item \textbf{Differentiable Mie lookup table} (\texttt{miejax\_lite}).
    A JAX-traceable table mapping $r_e \to (\omega, g_\ell)$ via
    pre-computed Mie coefficients with differentiable interpolation.

  \item \textbf{End-to-end gradient validation.}
    $\nabla_{\!\vect{x}} J$ via \texttt{jax.grad} through the full
    pipeline (lookup $\to$ Riccati ODE $\to$ BC solve $\to$ Fourier
    synthesis $\to$ cost) agrees with centred finite-difference
    gradients to ${\sim}2\times 10^{-10}$ relative error (float64).

  \item \textbf{AD mode.}  Forward mode (\texttt{jax.jacfwd} with
    \texttt{diffrax.ForwardMode}) is used for the implemented
    $p = 4$--$7$ parameter retrieval.

  \item \textbf{Retrieval loop.}  Gauss--Newton optimal estimation
    with autodiff Jacobians, QRCP grid selection, and posterior
    diagnostics (averaging kernels, DOFS, SIC).  Validated on
    VOCALS-REx marine stratocumulus profiles (thin and thick clouds,
    adiabatic and non-adiabatic).
\end{enumerate}

Open items tracked in the design documents include: sub-adiabatic
cloud-base detection (anchoring the $r_e^5$-linear extrapolation),
further investigation of the QRCP re-mesh instability (correlated
node basis at high $K$), the orthogonal shape/EOF basis alternative
to the ODE-pool route (\cref{sec:retrieval-grid}), and extension to
polarised observations (cloudbow geometry).
...
\bibitem{Wiscombe1977}
  ...
  \emph{J.\ Atmos.\ Sci.}, vol.~34, pp.~1408--1422, 1977.

\bibitem{LinStamnes2018}
  Z.~Lin, S.~Stamnes, Z.~Jin, I.~Laszlo, S.-C.~Tsay, W.~J.~Wiscombe,
  and K.~Stamnes,
  ``New treatment of strongly anisotropic scattering phase functions:
  The delta--$M$+ method,''
  \emph{J.\ Atmos.\ Sci.}, vol.~75, no.~1, pp.~327--336, 2018.

\bibitem{STWLE2000}
  K.~Stamnes, S.-C.~Tsay, W.~Wiscombe, I.~Laszlo, and K.~Evans,
  \emph{DISORT, a General-Purpose Fortran Program for Discrete-Ordinate-Method
  Radiative Transfer in Scattering and Emitting Layered Media: Documentation of
  Methodology}, 2000. (App.~A: the IMS correction applies to downward intensities only.)
