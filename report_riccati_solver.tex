...
\paragraph{Why step count is the objective --- and what it is \emph{not}.}
Minimising the number of $\tau$-points is a primary design objective, but
\emph{not} for run time: a per-step ESDIRK solve (several Newton
iterations, each an $O(N^3)$ factorisation) costs \emph{more} than a
Magnus step, so a $60\times$ step reduction is not a $60\times$ speed-up,
and in the current retrieval loop the dominant cost is in fact JIT
recompilation, not stepping.  The reason to minimise $\tau$-points is
\textbf{retrieval-grid viability}: the converged ODE grid is the only
a-priori-available, problem-derived, profile-adaptive candidate for the
grid on which $r_e(\tau)$ is retrieved (\cref{sec:retrieval-grid}).
Because the solver is L-stable, that grid clusters by \emph{genuine}
variation of the Riccati state --- not stiffness artifacts --- making it
a trustworthy \emph{superset} of the retrieval-informative depths while
remaining minimal, which is exactly what conditions the downstream
sensitivity selection (\cref{sec:retrieval-grid}).
...
A tempting alternative is to \emph{decouple} the retrieval grid from the
ODE grid entirely by parameterising $r_e(\tau)$ in a fixed smooth basis
(EOF/PCA, an adiabatic shape, or a neural decoder), evaluated at whatever
$\tau$ the solver requests.  We do not favour this: a fixed basis only
\emph{relocates} the a-priori assumption --- into an adiabatic shape that
fails for non-adiabatic clouds, or a training distribution with
extrapolation risk --- and it is \emph{static}, whereas the ODE grid is
problem-derived and re-placeable within the loop.  The ODE-pool approach
does inherit the \emph{first-guess} profile's bias in its placement, but
that bias is correctable by lagged grid re-selection (re-run the forward
model, re-place the pool), which a frozen basis cannot do.
